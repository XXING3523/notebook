...
定义：$X$是概率空间上的实值随机变量，定义$X$的特征函数$\phi_X:\R\to\C$为：$\phi_X(t)=E(e^{itX},t\in\R)$
...
Taylor展开：若对某个$n\in\N,E|X^n|<+\infty$，则：$\displaystyle \phi_{T_k}(t)=\sum_{p=0}^{n}\frac{(it)^p}{p!}E(X_k^p)+O(t^n)$，记$\mu_p'=E(X_k^p)$
