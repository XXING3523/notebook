...
**优点**：存储利用率高（没有额外的指针开销，数据元素紧密排列，空间利用率高），存取速度快；**缺点**：插入、删除效率低，空间分配不灵活（需预先分配固定大小的空间，可能导致空间浪费（预分配过大）或溢出（预分配过小））

**链式存储方式（链表）**

**优点**：适应表的动态增长和删除（链表的空间是动态申请和释放的，能有效利用存储空间，没有固定大小的限制，按需分配。）（可以很方便地扩充与收缩），结点可以不连续存储，插入和删除操作效率高；

**缺点**：存储密度低（需要额外的指针存储空间），存取效率低（只能从表头开始顺序访问元素，无法随机存取）
...
{
     CircLinkNode<E> *p=Js.getHead(),*pre=NULL;
     //p为第一个人
     int i, j;
     for(i=0;i<n-1;i++) 
     {     
...
{		
     ...
     cout<<“输入游戏者人数和报数间隔:”;
     cin>>n>>m;
     for(i=1;i<=n;i++) clist.insert(i,i);    
     Josephus(clist,n,m);                 
}
